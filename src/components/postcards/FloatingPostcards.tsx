...
export default function FloatingPostcards() {
    ...
    async function loadPostcards() {
      ...
      if (!user) {
        const { data } = await supabase
          .from('postcards').select('*').order('created_at', { ascending: true }).limit(10);
      ...
      if (asRequester) asRequester.forEach((f) => friendIds.push(f.addressee_id));
      if (asAddressee) asAddressee.forEach((f) => friendIds.push(f.requester_id));

      const { data } = await supabase
        .from('postcards').select('*').in('user_id', friendIds)
  ...
  return (
    <>
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        {postcards.map((postcard, idx) => {
          ...
          return (
            ...
            >
              {/* No caption shown on the thumbnail — image only */}
      ...
      {selected && (
        <PostcardModal postcard={selected} onClose={() => setSelected(null)} />
      )}
    </>
  );
}