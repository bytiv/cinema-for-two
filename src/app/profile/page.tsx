...
import { User, Camera, Save, Plus, X, Film, Image as ImageIcon, History, Users } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  async function loadData() {
    ...
    // Fetch accepted friendships in two queries (avoids .or() RLS issues)
        ...
        <div className="space-y-6">
          ...
          <div className="bg-cinema-card/50 backdrop-blur-sm border border-cinema-border rounded-2xl p-6">
            <h3 className="font-display text-lg font-semibold text-cinema-text flex items-center gap-2 mb-4">
              ...
              Friends
              {friends.length > 0 && (
                <span className="text-sm font-normal text-cinema-text-muted">({friends.length})</span>
              )}
            </h3>
            ...
            {friends.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {friends.map((f) => {
                  ...
                  return (
                    <div key={f.id} className="flex items-center gap-3 p-3 rounded-xl bg-cinema-surface/60 border border-cinema-border/50">
                      <div className="relative flex-shrink-0">
                        ...
                        <div className={cn(
                          'absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-cinema-surface',
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-cinema-text-dim truncate">
                          {online ? (
                            <span className="text-cinema-success">Online</span>
                          ) : fp.last_seen_at ? (
                            formatRelativeTime(fp.last_seen_at)
                          ) : 'Offline'}
                        </p>
                      </div>
                    </div>
            ...
            {postcards.length === 0 ? (
              ...
              <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                {postcards.map((pc) => (
                  <div
                    key={pc.id}
                    className="relative group rounded-lg overflow-hidden aspect-[3/4] bg-cinema-surface cursor-pointer"
