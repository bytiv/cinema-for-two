...
import { Upload, Film, Image as ImageIcon, X, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function UploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Step 1: Get SAS URL for movie upload
      ...
      const sasRes = await fetch('/api/upload/sas', {
        ...
        body: JSON.stringify({
          container: 'movies',
          blobName,
          contentType: getVideoMimeType(movieFile.name),
        }),
      });
      ...
      // Step 2: Upload movie to Azure Blob using PUT with SAS URL
      ...
      // Step 4: Save movie metadata to Supabase
      ...
      // Save via API route (which generates proper SAS URLs for reading)
      ...
      <main className="relative z-10 pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-2xl mx-auto">
        ...
        <div className="space-y-6">
          ...
          <div
            className={cn(
              ...
              dragActive
                ...
                : movieFile
                  ? 'border-cinema-success/50 bg-cinema-success/5'
          ...
          >
            ...
            {movieFile ? (
              <div className="flex items-center justify-center gap-4">
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); setMovieFile(null); }}
                  className="ml-4 text-cinema-text-dim hover:text-cinema-error transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
          ...
          <div className="bg-cinema-card/50 backdrop-blur-sm border border-cinema-border rounded-2xl p-6 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-start gap-4">
                ...
                <p className="text-xs text-cinema-text-dim mt-2">
                  A poster helps identify the movie in the library. JPG, PNG, or WebP under 10MB.
                </p>
              </div>
            </div>
          </div>
          ...
          {uploading && (
            <div className="space-y-3">
              ...
              <div className="h-2 bg-cinema-card rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-cinema-accent to-cinema-warm rounded-full transition-all duration-500 ease-out"
          ...
          <Button
            ...
            disabled={!movieFile || !title.trim() || uploading}
            loading={uploading}
            size="lg"
            className="w-full"
